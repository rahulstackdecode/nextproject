...
const MobileNav = ({ closeNav, showNav }: Props) => {
    ...
    return (
        ...
        bg-black space-y-6 z-[100] d-none`} >
                {navLinks.map((link) => {
                    return <Link href={link.url} key={link.id}><p className='nav_link text-white text=[20px] ml-12 border-b=[1.5px] pb-1 border-white sm:text=[30px]'>{link.label}</p></Link>
                })}
